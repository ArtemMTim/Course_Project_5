...
class HabitValidator:
    ...
    def __call__(self, value):
        ...
        if value["related_habit"] and value["reward"]:
            raise serializers.ValidationError("Нельзя выбирать связанную привычку и вознаграждение одновременно.")

        if value["is_pleasant"]:
            if value["reward"] or value["related_habit"]:
                raise serializers.ValidationError("Нельзя выбирать связанную привычку или вознаграждение для приятной привычки.")
